    ...
    itemHtml += `${
      isThereSubs
        ? `<span class="ct-parent-plus">${
            this.options.collapse || collapse ? `<i class="fas fa-caret-right"></i>` : `<i class="fas fa-caret-down"></i>`
          }</span>`
        : ""
    }<span
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined) {
      if ($(subMenu).is(":visible"))
        $(item).children("span.ct-parent-plus").html(`<i class="fas fa-caret-right"></i>`);
      else $(item).children("span.ct-parent-plus").html(`<i class="fas fa-caret-down"></i>`);

      $(subMenu).slideToggle(this.options.animationTime);
    } else if (direction == 1 && !$(subMenu).is(":visible")) {
      $(item).children("span.ct-parent-plus").html(`<i class="fas fa-caret-down"></i>`);
